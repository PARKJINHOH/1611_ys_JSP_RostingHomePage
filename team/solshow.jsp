			...
			<div class="full-wrapper relative clearfix">
				<div class="left" style="width: 15%; height: 100% px; float: left;">
					...
					<ul>
						...
						<li><a href="solshow.jsp">질문게시판</a></li>
...
<%
    ...
    // 읽은 횟수 하나 증가
       strSql = "update forum set hit = hit + 1 where id = " + strId; 
...
<p> 
[<A href="solution.jsp?page=<%=request.getParameter("page")%>">게시판으로 돌아가기</
A>][<A href="solupdate.jsp?id=<%=strId%>&mode=show&page=<%=request.getParameter("page"
)%>">글수정</A>][<A href="soldelete.jsp?id=<%=strId%>&page=<%=request.getParameter("page")%>">
글삭제</A>]
[<A href="solwriteform.jsp?id=<%=strId%>&page=<%=request.getParameter("page")%>&
refid=<%=rs.getInt("refid")%>&lev=<%=rs.getInt("lev")%>&pos=<%=rs.getString("pos")%>">답변하기</A>]
<%
       } // end of if
     } catch (Exception e) {
